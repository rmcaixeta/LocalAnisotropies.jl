...
@testset "LocalAnisotropies.jl" begin
    ...
    pts3d = rand(3, 10)
    dummy = georef(
        (a1 = 1:10, a2 = 1:10, a3 = 1:10, r1 = 1:10, r2 = 1:10, r3 = 1:10),
        PointSet([tuple(pts3d[:, x]...) for x = 1:10]),
    )
    pars = localanisotropies(dummy, [:a1, :a2, :a3], [:r1, :r2, :r3], :Datamine)
    pars_ = adjust_rake(pars, [45 + i for i = 1:10])
    @test all(
        round.(rotmat(pars, 1)[3, :], digits = 4) .≈
        round.(rotmat(pars_, 1)[3, :], digits = 4),
    )
    @test !all(
        round.(rotmat(pars, 1)[1, :], digits = 4) .≈
        round.(rotmat(pars_, 1)[1, :], digits = 4),
    )
    ...
    pset = PointSet([tuple(data[:, x]...) for x = 1:100])
    ...
    for dims in (grid2d, grid3d)
        ...
        R = length(dims)
        D = if R == 2
            georef((P = [sin(i) + j for i = 1:dims[1], j = 1:dims[2]],))
        else
            georef((P = [sin(i) + j + k for i = 1:dims[1], j = 1:dims[2], k = 1:dims[3]],))
        end
        ...
        expvario =
            localvariography(S, spars, :P, tol = 2, maxlag = 20, nlags = 20, axis = :X)
