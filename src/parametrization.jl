...
centro(D::LocalGeoData, i::Int) =
    i <= nvals(D) ? centro(obj(D), i) : centro(sobj(D), i - nvals(D))
coords_(D::LocalGeoData, i::Int) = ustrip.(to(centro(D, i)))
coords_(D::SpatialData) = reduce(hcat, [ustrip.(to(centro(D, x))) for x = 1:nvals(D)])
coords_(D::SpatialData, i::AbstractVector{Int}) =
    reduce(hcat, [ustrip.(to(centro(D, x))) for x in i])
...
rotation(D::LocalGeoData, i::Int) =
    i <= nvals(D) ? rotation(D)[i] : rotation(D)[spars(D, i)]
rotation(D::LocalGeoData, i::AbstractVector{Int}) =
