...
@recipe function f(lpars::LocalAnisotropy, D::SpatialData, axis = :Y)
    ...
    u = [quat_to_dcm(quats[x])[1, 1] for x = 1:length(quats)]
    v = [quat_to_dcm(quats[x])[1, 2] for x = 1:length(quats)]
    x = [to(centro(D, x))[1] for x = 1:nvals(D)]
    y = [to(centro(D, x))[2] for x = 1:nvals(D)]
